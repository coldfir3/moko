...
vignette: |
  %\VignetteIndexEntry{Multiobjective Optimization of the Nowacki Beam} %\VignetteEngine{knitr::rmarkdown} %\VignetteEncoding{UTF-8}
---
...
\begin{aligned}
	...
	\text{subject to 5:}    &\: F_\text{crit} = -\frac{4}{l^2}\sqrt{G\frac{(b^3h+hb^3)}{12}E\frac{b^3h}{12}\frac{1}{(1-v^2)}} \leq -2F, \nonumber
\end{aligned}
$$
...
The quality of the Pareto sets found are compared using the inverted generational distance (IGD) metric [@shimoyama2013kriging]. The IGD can be defined as
$$
...
par(mar=c(3,2.5,0.5,0.5)+0.1,mgp = c(1.5, 0.75, 0))
plot(true_ps$set, pch=20, xlab='Area', ylab='Bending Stress', col='blue', cex=0.2)
```
...
library(lhs)
```
After loading the necessary packages, we generate an initial DOE using an optimized Latin hypercube of `n` samples and two dimmensions.
```{r}
n <- 20
d <- 2
doe <- optimumLHS(n,d)
...
```
Then, we create a multi-objective kriging model calling the function `mkm`. Note that we need to tune the `modelcontrol` argument in order to tell the function that our data have two objectives. By doing that, the remaining columns of the response will be flagged as constraints. Also, in order to increas stability, we set the lower bounds for the kriging hyperparameter estimation as `0.1` for all variables (for more information check the *Identifiability issues caused by large design interdistances* section of [@roustant2012dicekriging])
```{r}
model <- mkm(doe, res, modelcontrol = list(objective = 1:2, lower = rep(0.1, d)))
```
Now that we executed step 1 and 2 of any of the 3 optimization techniques presented here, we will apply the VMPF algorithm on the initial model to demonstrate how to handle the `mkm` object.

Considering a total budget of 40 evaluations (which 20 were already spent building the initial model) we can code the technique as follows:
```{r}
for (i in 21:40){
...
Alternatively, one can use the `VMPF` function and save some coding lines. This funcion is basicly a wraper for the demonstraded algorithm, it recives the number of iterations and a `mkm` model as input and returns the updated model. There are also wrappers for the other two algorthms that can be used as follows:
```r
model <- mkm(doe, res, modelcontrol = list(objective = 1:2, lower = rep(0.1, d)))
niter <- 20
